...
from app.api.v1.modules.masters.services.provinces_search_service import ProvinceSearchService
from app.api.v1.modules.masters.models._envelopes import ProvinceSearchEnvelope
...
def get_search_service(session: AsyncSession = Depends(get_db)) -> ProvinceSearchService:
    return ProvinceSearchService(ProvinceSearchRepository(session))
...
@router.get(
    ...
    operation_id="grid_provinces",
)
async def grid_provinces(
    request: Request,
    q: str = Query("", description="Search keyword"),
    country_code: str | None = Query(None, description="Filter by country_code"),
    ...
    svc: ProvinceSearchService = Depends(get_search_service),
):
    ...
    items = [ProvinceResponse.model_validate(_normalize_row(r), from_attributes=True).model_dump(exclude_none=True) for r in rows]
