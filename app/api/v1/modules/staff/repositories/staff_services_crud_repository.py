...
class StaffServicesCrudRepository:
    def __init__(self, db: AsyncSession):
        self.db = db

    async def create(self, data: dict) -> StaffService:
    ...
    async def update(self, staff_service_id: UUID, data: dict) -> StaffService | None:
        obj = await self.db.get(StaffService, staff_service_id)
        if not obj:
            return None
