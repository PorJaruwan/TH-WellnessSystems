    ...
    responses={
        **success_200_example(
            example=success_example(
                ...
                data={"filters": {"q": ""}, "paging": {"total": 0, "limit": 50, "offset": 0}, "geographies": []},
            )
        ),
...
async def search_geographies(
    ...
    q: str = Query(default="", description="keyword (like): id / name"),
    ...
    Response data shape:
    - {"filters": {...}, "paging": {"total": N, "limit": L, "offset": O}, "geographies": [...]}
    """
    filters = {"q": q}
    ...
    async def _work():
        ...
        if q:
            kw = f"%{q}%"
            where.append(or_(cast(Geography.id, String).ilike(kw), Geography.name.ilike(kw)))
