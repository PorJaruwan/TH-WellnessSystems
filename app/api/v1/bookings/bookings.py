...
async def search_patients(
    ...
    result = await search_patients_service(
...
@router.get("/staff/search", response_model=StaffListResponse)
async def search_staff(
    ...
    result = await search_staff_service(
...
async def create_booking(body: BookingCreate = Body(...)):
    return await create_booking_service(booking=body)


@router.get("/{booking_id}", response_model=BookingDetail)
async def get_booking_detail(booking_id: UUID):
    return await get_booking_detail_service(booking_id=booking_id)


@router.patch("/{booking_id}/note")
async def update_booking_note(
    booking_id: UUID,
    body: BookingUpdateNote = Body(...),
):
    await update_booking_note_service(
...
async def get_booking_history(booking_id: UUID):
