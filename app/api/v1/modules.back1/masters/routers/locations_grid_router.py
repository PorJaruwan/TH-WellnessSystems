...
from app.api.v1.modules.masters.services.locations_search_service import LocationSearchService
from app.api.v1.modules.masters.models._envelopes import LocationSearchEnvelope
...
def get_search_service(session: AsyncSession = Depends(get_db)) -> LocationSearchService:
    return LocationSearchService(LocationSearchRepository(session))
...
@router.get(
    ...
    operation_id="grid_locations",
)
async def grid_locations(
    request: Request,
    ...
    svc: LocationSearchService = Depends(get_search_service),
):
    ...
    items = [LocationResponse.model_validate(_normalize_row(r), from_attributes=True).model_dump(exclude_none=True) for r in rows]
