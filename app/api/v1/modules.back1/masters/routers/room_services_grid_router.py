...
from app.api.v1.modules.masters.services.room_services_search_service import RoomServiceSearchService
from app.api.v1.modules.masters.models._envelopes import RoomServiceSearchEnvelope
...
def get_search_service(session: AsyncSession = Depends(get_db)) -> RoomServiceSearchService:
    return RoomServiceSearchService(RoomServiceSearchRepository(session))
...
@router.get(
    ...
    operation_id="grid_room_services",
)
async def grid_room_services(
    request: Request,
    q: str = Query("", description="Search keyword"),
    room_id: UUID | None = Query(None, description="Filter by room_id"),
    service_id: UUID | None = Query(None, description="Filter by service_id"),
    ...
    svc: RoomServiceSearchService = Depends(get_search_service),
):
    ...
    items = [RoomServiceResponse.model_validate(_normalize_row(r), from_attributes=True).model_dump() for r in rows]
    # items = [RoomServiceResponse.model_validate(_normalize_row(r), from_attributes=True).model_dump(exclude_none=True) for r in rows]
