...
from app.api.v1.modules.staff.models.staff_model import StaffAvailabilityCreateModel, StaffAvailabilityUpdateModel
from app.api.v1.modules.staff.models.staff_response_model import StaffAvailabilityResponse
...
async def read_staff_availability_by_id(staff_availability_id: UUID, session: AsyncSession = Depends(get_db)):
    try:
        ...
        return ApiResponse.ok(
            success_key="RETRIEVED",
            default_message="Retrieved successfully.",
            data={"item": StaffAvailabilityResponse.model_validate(obj).model_dump(exclude_none=True)},
        )
...
async def delete_staff_availability_by_id(staff_availability_id: UUID, session: AsyncSession = Depends(get_db)):
    try:
        ...
        return ApiResponse.ok(
            ...
            data={"item": str(staff_availability_id)},
        )
